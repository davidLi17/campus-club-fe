...
const processApplicationData = () => {
  ...
  // 统计各阶段数量（后端状态为小写）
  const pending = props.applications.filter(
    (app) => app.status?.toLowerCase?.() === "pending"
  ).length;
  const interviewing = props.applications.filter(
    (app) => app.status?.toLowerCase?.() === "interviewing"
  ).length;
  const approved = props.applications.filter(
    (app) => app.status?.toLowerCase?.() === "approved"
  ).length;
  const joined = props.applications.filter(
        ...
        data: funnelData.map((item, index) => ({
          ...
          itemStyle: {
            color: new echarts.graphic.LinearGradient(0, 0, 1, 0, [
              {
                offset: 0,
                color: ["#667eea", "#48bb78", "#ed8936", "#f687b3", "#38b2ac"][
                  index
                ],
              },
              {
                offset: 1,
                color: ["#764ba2", "#38a169", "#dd6b20", "#d53f8c", "#319795"][
                  index
                ],
              },
            ]),
